...
void rebalanceTree(Tree* t, TNode* x){
    while ( x != NULL )
    {
        ...
        {
            TNode* z = subTreeHeight( x->pLeft ) > subTreeHeight( x->pRight ) ? x->pLeft : x->pRight;
            // if the balace of x and the balance of z have different signs
            if( getBalance( z ) > 0 )
            {
                rightRotate( t, z );
            }
...
void printHuffmanEncoding( TNode* root, char c ){
    ...
    else if( strchr( root->pLeft->str, c ) != NULL )
    {
        printf("0");
        root = root->pLeft;
        printHuffmanEncoding( root, c );
    }
...
void insertSegment( TNode* root, double segmentStart, double segmentEnd ){
    ...
    else if( root->high < segmentStart || root->low > segmentEnd )
    {
        return;
    }
...
int lineStabQuery( TNode* root, double queryPoint ){
    ...
    {
        sum = root->cnt;
        sum += lineStabQuery( root->pLeft, queryPoint );
        sum += lineStabQuery( root->pRight, queryPoint );
        return sum;
    }
}
